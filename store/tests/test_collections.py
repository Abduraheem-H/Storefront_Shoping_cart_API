...
class TestDeleteCollection:
    ...
    def test_if_collection_does_not_exist_returns_404_on_delete(
        self, api_client: APIClient, authenticate
    ):
        authenticate(is_staff=True)
        response = api_client.delete(reverse("store:collection-detail", args=[9999]))
        assert response.status_code == status.HTTP_404_NOT_FOUND
...
class TestListCollections:
    ...
    def test_if_collections_does_not_exist_returns_404(self, api_client: APIClient):
        response = api_client.get(reverse("store:collection-list"))
        assert response.status_code == status.HTTP_200_OK
...
class TestUpdateCollection:
    ...
    def test_if_collection_does_not_exist_returns_404_on_update(
        self, api_client: APIClient, authenticate
    ):
        authenticate(is_staff=True)
        updated_data = {"title": "Updated Collection Title"}
        ...
        response = api_client.put(
            reverse("store:collection-detail", args=[9999]),
            data=updated_data,
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_if_payload_is_invalid_returns_400_on_update(
        self, api_client: APIClient, authenticate
    ):
        authenticate(is_staff=True)
        collection = baker.make("store.Collection")
        invalid_data = {"title": ""}  # Empty title to trigger validation error
    ...
    def test_if_user_is_anonymous_returns_401_on_update(self, api_client: APIClient):
        collection = baker.make("store.Collection")
        updated_data = {"title": "Updated Collection Title"}
